...
## 2026-06-02：状态文档压缩保留后续影响信息
**决策**：压缩 `PROGRESS.md`、`DECISIONS.md`、`CHANGELOG.md` 时，只保留当前阶段、仍有效的模块级摘要、后续入口、架构/维护策略决策和用户可感知或部署相关变更。
**原因**：状态文档用于跨会话恢复上下文，重复流水和已解决排查过程会降低检索效率；保留仍影响后续工作的内容可避免丢失维护边界。
**否决方案**：保留所有历史流水（原因：噪音过多）；只保留最近一次任务（原因：会丢失仍有效的技术栈、导航、推送和来源追溯约束）。
**影响范围**：`PROGRESS.md`、`DECISIONS.md`、`CHANGELOG.md`

## 2026-06-02：Codex CLI 命令参考按主题拆分
**决策**：将 `wiki/codex-cli-command-reference.md` 作为主题总览页，把全局参数、顶层命令、slash commands 和常用工作流拆分到 `wiki/codex-cli/` 子页面。
**原因**：Codex CLI 内容覆盖多个自然章节，按“总索引 -> 主题总览 -> 子页面”维护更便于扩展和检索。
**否决方案**：继续单页维护（原因：内容增长后检索效率下降）；把所有子页面加入 `wiki/index.md`（原因：总索引只维护一级主题入口）。
**影响范围**：`wiki/codex-cli-command-reference.md`、`wiki/codex-cli/`、`wiki/log.md`

## 2026-06-02：Wiki 总索引只维护一级主题入口
**决策**：`wiki/index.md` 只维护一级主题入口；大主题通过总览页继续拆分到子页面，并要求总览页与子页面双向链接。
**原因**：随着 Wiki 体量增长，所有细节页都放进总索引会降低可读性；分层结构可以减少导航噪音。
**否决方案**：在 `wiki/index.md` 直接列出所有细节页面（原因：数据量大后索引臃肿）；不允许主题拆分（原因：单页过长会降低维护和检索效率）。
**影响范围**：`AGENTS.md`、`wiki/index.md`、后续 Wiki 页面拆分与导航维护流程

## 2026-06-02：AGENTS 只保留 Wiki 入口
**决策**：`AGENTS.md` 只保留 `wiki/index.md` 与 `wiki/log.md` 入口，具体主题导航统一维护在 `wiki/index.md`。
**原因**：重复维护 `AGENTS.md` 和 `wiki/index.md` 两份主题导航容易遗漏同步；`AGENTS.md` 的职责是维护规则和入口。
**否决方案**：继续在 `AGENTS.md` 逐项列出主题页面（原因：新增页面时需要双处更新）；完全删除入口说明（原因：代理开始维护时仍需要明确入口文件）。
...
**原因**：当前长期归档来源数量较少，`raw/`、页面 frontmatter `sources` 和 `wiki/log.md` 已能支撑追溯；先沉淀规则可避免过早增加维护页面。
**否决方案**：立即新建 `wiki/source-index.md`（原因：当前来源规模不足）；仅依赖聊天记录记忆（原因：无法稳定反查原始文件影响范围）。
...
**决策**：树形上下级级联数据的展开状态应按稳定节点标识维护，数据刷新和增删改后恢复此前布局；通用前端经验页和 Vue 3 专项规范同步记录。
**原因**：树形展开状态属于用户正在操作的界面上下文，增删改或刷新后重置会打断连续操作。
**否决方案**：仅依赖 UI 组件默认展开行为（原因：默认行为常在数据源重载后丢失状态）；仅点击展开图标（原因：不符合整行点击体验要求）。
**影响范围**：`wiki/frontend-development-experience.md`、`wiki/vue3-frontend-development-guide.md`

## 2026-05-29：前端列表展示约定同时维护在通用页与 Vue 3 页
**决策**：将“列表标题与列表内容居中展示”写入通用前端经验页，并同步补充到 Vue 3 实施与验证要求。
**原因**：该要求属于通用前端视觉规则，同时当前存在 Vue 3 专项规范，双处记录可覆盖规范查询与具体实施。
...
**否决方案**：继续将所有规范放在根目录单独文件中（原因：缺少来源边界、索引与积累工作流）。
...
**原因**：同一功能的一致性要求需要单一来源，避免前后端页面分别维护后产生不同解释。
**否决方案**：仅在前端和后端页面分别重复写入规则（原因：重复内容容易出现更新遗漏和规则分叉）。
**影响范围**：`wiki/cross-page-feature-consistency.md`、`wiki/frontend-development-experience.md`、`wiki/python-backend-development-guide.md`
...
**影响范围**：`AGENTS.md`、`wiki/python-backend-development-guide.md`、Git 操作流程

## 2026-05-26：后端规范按 Python 技术栈命名
**决策**：将实际维护页面更名为 `wiki/python-backend-development-guide.md`，标题改为“Python 后端开发规范”；旧 Wiki 路径保留跳转入口。
...
## 2026-05-26：前端采用 Vue 3 与 TypeScript
**决策**：将用户提出的 `Vu3` 按上下文规范为 `Vue 3`；Vue 3 前端开发使用 `TypeScript`，组件边界、共享逻辑和 API 请求响应均以显式类型表达。
**原因**：Vue 3 和 `ts` 已由用户指定；专项页面可明确组件、状态、接口契约和联调规则。
**否决方案**：仅在通用前端经验页面零散记录 Vue 3 要求（原因：无法清晰表达框架适用范围）；继续将 JavaScript 与 TypeScript 都列为待选项（原因：与已明确要求冲突）。
