...
## 2026-06-02：Codex CLI 命令参考拆分为主题总览和子页面
**决策**：将 `wiki/codex-cli-command-reference.md` 改为主题总览页，并把全局参数、顶层命令、slash commands 和常用工作流拆分到 `wiki/codex-cli/` 子页面
**原因**：Codex CLI 内容已经覆盖多个自然章节，单页维护会变长；按最新分层导航规则拆分后，总览页负责入口和摘要，子页面负责细节，便于后续按主题更新
**否决方案**：继续把所有内容放在单个 `codex-cli-command-reference.md`（原因：内容继续增长会降低检索和维护效率）；把所有子页面加入 `wiki/index.md`（原因：总索引只维护一级主题入口）
**影响范围**：`wiki/codex-cli-command-reference.md`、`wiki/codex-cli/`、`wiki/log.md`

## 2026-06-02：Wiki 总索引只维护一级主题入口
**决策**：规定 `wiki/index.md` 只维护一级主题入口，具体大主题可通过主题总览页继续拆分到子页面，并要求总览页与子页面双向链接
**原因**：随着 Wiki 体量增长，所有细节页都放进总索引会降低可读性；通过“总索引 -> 主题总览 -> 子页面”的层级结构，可以减少导航噪音并支持大主题继续扩展
**否决方案**：在 `wiki/index.md` 直接列出所有细节页面（原因：数据量大后索引臃肿）；不允许主题拆分（原因：单页过长会降低维护和检索效率）
**影响范围**：`AGENTS.md`、后续 Wiki 页面拆分与导航维护流程
